<template>
	<view class='issue'>
		<view class="issue-head">
			...
			<view class="issue-head-star-box" style="float: right;margin-top: 10px;">
				<checkbox-group class="uni-list" @change="checkboxChange">
					<label class="radio" style="line-height: 30px;">
						<checkbox value="" style="border-radius: 50%;" color="#fa436a" :checked='isCheck' />
							<text>匿名</text>
						</checkbox >
					</label>
				</checkbox-group>
				<!-- <text class="yticon icon-look" v-if="item.is_anonymous === false" @click.stop="addAddress(item)"></text> -->
			</view>
		 ...
		 <view class="issue-btn-box">
		 	<button v-if="submitShow" class="submit-btn" type="primary" @click="doSubmit" submitText="评价">{{submitText}}</button>
...
<script>
	export default {
		...
		methods: {
			...
			checkboxChange(){
				this.$emit('checkboxChange',this.isCheck)
			},
