			...
			<view class="aside">
				<view class="content">
					...
					<view class="input-content" v-if="showCode">
						<view class="wrap">
							<view class="input-item">
								<input :value="username" placeholder="请输入手机号" maxlength="20" data-key="username"
									@input="inputChange" />
							</view>
							<view class="errorMsg">
						...
						<view class="wrap">
							<view class="input-item">
								<input :value="username" placeholder="请输入手机验证码" maxlength="20" data-key="username"
									@input="inputChange" />
								<view style="position: absolute;right: 40px;" v-if="send">
									<text type="text" @click="mobileCode"
										style="color: #999;font-size: 12px;padding: 0 20px;border-left: 1px solid #999;">获取验证码</text>
								</view>
								<view style="position: absolute;right: 50px;" v-else>
									<text type="text" disabled="" @click="mobileCode"
										style="color: #999;font-size: 12px;height: 30px;line-height: 30px;">{{time}}s后重新获取</text>
								</view>
							</view>
							<view class="errorMsg">
					...
					<view class="input-content" v-else>
						<view class="wrap">
							<view class="input-item">
								<input :value="username" placeholder="请输入用户名/手机号" maxlength="20" data-key="username"
									@input="inputChange" />
							</view>
							<view class="errorMsg">
								<text class="message" v-if="phone">请输入正确的手机号码</text>
						...
						<view class="wrap">
							<view class="input-item">
								<input type="password" :value="password" placeholder="请输入密码" maxlength="20" password
			...
			<view class="login" style="margin-bottom: 50px;">
				...
				<view class="otherLogin">
					<view class="weiixnLogin" @click="WX_login">
						<image style="width: 60upx;height: 50upx;"
							src="http://47.94.106.106:8888/group1/M00/5D/27/rBHxiGGts5-ARb9cAAAFyaS9Bh84992368" mode="">
						</image>
					</view>
	...
	export default {
		...
		methods: {
			...
			mobileCode() {
				...
				uniRequest.get('/sms_codes/' + this.mobile + '/?text=' + this.getText + '&image_code_id=' + this
					.image_code_id).then(res => {
					if (res.status === 200) {
						var num = setInterval(() => {
							this.send = false
							this.time -= 1
			...
			async toLogin() {
				...
				console.log(username, password, this.checked)
				if (this.checked) {
					const sendData = {
						username: username,
						password: password,
