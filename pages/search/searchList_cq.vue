	...
	<view class="">
		<view class="search">
			...
			<template v-else>
				...
				<!-- <view class="head">
					<text class="yticon icon-zuo" style="position: absolute;left: -25px;top: 0;line-height: 30px;font-size: 24px;color: #000;" @click="goBack"></text>
					
					<text class="yticon icon-gouwuche" style="position: absolute;right: -35px;top: 0;line-height: 30px;font-size: 20px;" @click="goToPage"></text>
				</view> -->
			</template>
		...
		<view class="guess-section">
			...
			>
				<view class="image-wrapper">
					<image style="border: 1px solid #ddd;" :src="item.image1" mode="aspectFill"></image>
				</view>
				<text class="title clamp">{{item.name}}</text>
				<view class="">
					<text class="price" style="float: left;">￥{{item.price}}</text>
					<text style="font-size: 12px;float: right;color: #666;">1000人付款</text>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods:{
			...
			async getDate(type){
				...
				try {
					const response = await uniRequest.get('/groups/search/goods/?name='+this.searchText+'&page='+this.page+'&page_size='+this.page_size);
