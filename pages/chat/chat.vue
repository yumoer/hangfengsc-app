		...
		<view class="content" @touchstart="hideEmoji">
			<scroll-view class="msg-list" scroll-y="true" :scroll-with-animation="scrollAnimation" :scroll-top="scrollTop" :scroll-into-view="scrollToView">
				<view class="row" v-for="(row,index) in msgList" :key="index" :id="'msg'+row.id">
					<!-- 别人发出的消息 -->
					<view class="other" v-if="row.username === '售后客服'">
						<view class="left">
							<image :src="row.face"></image>
						</view>
						<view class="right">
							<view class="username">
								<view class="name">{{row.username}}</view> <view class="time" style="line-height: 18px;">{{row.time}}</view>
							</view>
							<view v-if="row.type=='str'" class="bubble">
	...
	export default {
		...
		methods:{
			...
			sendMsg(content,type){
				...
				// console.log(lastid)
				lastid++;
				let msg = {id:lastid,username:uni.getStorageSync('userInfo').username,face:"http://47.94.106.106:8888/group1/M00/5D/29/rBHxiGGtv3iAL6TfAAAiiPdJBLU5098017",time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,msg:content};
				// console.log(msg)
				this.screenMsg(msg);
				this.clickRequest()
				/* // 定时器模拟对方回复,三秒
